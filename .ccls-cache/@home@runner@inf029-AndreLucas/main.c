...
void getDataStudent(Disciplina *A, int dis, int *qtd_std, int std) {
  ...
  A[dis].student[qtd_std[dis]].sex = studentsList[std].sex;
  A[dis].student[qtd_std[dis]].dateOfBirth[0] = studentsList[std].dateOfBirth[0];
  A[dis].student[qtd_std[dis]].dateOfBirth[1] = studentsList[std].dateOfBirth[1];
  A[dis].student[qtd_std[dis]].dateOfBirth[2] = studentsList[std].dateOfBirth[2];
}
...
void changeTeacher(Teacher vetor[], int vetor2[]) {
  int i, index;
  int check = 0;
  while (check == 0) {
    int matricula;
    ...
    for (i = 0; i < LEN_TEACHER; i++) {
      if (vetor[i].registration == matricula && vetor[i].status == 1) {
        index = i;
        check = 1;
        break;
      } else
        check = 0;
      if (check == 0)
        printf("Professor não encontrado, digite a matricula novamente!");
    }
  }
  int stop = 0;
  while (stop == 0) {
    int op;
    printf("\n<<<ESCOLHA UMA CARACTERISTICA PARA EDITAR>>>\n");
    printf("1-> Matricula\n");
    printf("2-> Nome\n");
    printf("3-> CPF\n");
    printf("4-> Sexo\n");
    ...
    case 1:
      ...
      scanf("%d", &vetor[index].registration);
    ...
    case 2:
      ...
      fgets(vetor[index].name, 50, stdin);
    ...
    case 3:
      ...
      fgets(vetor[index].cpf, 15, stdin);
    ...
    case 5:
      ...
      printf("Dia:\n ");
      scanf("%d", &vetor[index].dateOfBirth[0]);
      vetor2[index] = vetor[index].dateOfBirth[0];
      printf("Mês:\n ");
      scanf("%d", &vetor[index].dateOfBirth[1]);
      vetor2[index] = vetor2[index] + vetor[index].dateOfBirth[1] * 100;
      printf("Ano:\n ");
      scanf("%d", &vetor[index].dateOfBirth[2]);
      vetor2[index] = vetor2[index] + vetor[index].dateOfBirth[2] * 10000;
      printf("Data de Nascimento editada com sucesso!\n");
      break;
    }
  }
}

void RegisterTeacher(Teacher vetor[], int vetor2[], int i) {
  ...
  while (option != 1) {
    ...
    printf("-----------------------------------\n");
    printf("Matricula: ");
    scanf("%d", &vetor[i].registration);
    getchar();
    printf("Nome do Professor: ");
    fgets(vetor[i].name, 50, stdin);
    printf("Sexo: ");
    scanf("%c", &vetor[i].sex);
    getchar();
    printf("Data de Nascimento>>>\n");
    printf("Dia: ");
    scanf("%d", &vetor[i].dateOfBirth[0]);
    getchar();
    vetor2[i] = vetor[i].dateOfBirth[0];
    printf("Mês: ");
    scanf("%d", &vetor[i].dateOfBirth[1]);
    getchar();
    vetor2[i] = vetor2[i] + vetor[i].dateOfBirth[1] * 100;
    printf("Ano: ");
    scanf("%d", &vetor[i].dateOfBirth[2]);
    getchar();
    vetor2[i] = vetor2[i] + vetor[i].dateOfBirth[2] * 10000;
    vetor[i].date = vetor2[i];
    printf("CPF: ");
    fgets(vetor[i].cpf, 15, stdin);
    vetor[i].status = 1;
    i++;
    printf("PROFESSOR CADASTRADO COM SUCESSO!\n");
    printf("DESEJA CADASTRAR NOVO PROFESSOR? (DIGITE 0 -> SIM | DIGITE 1-> "
           "NÃO)\n");
    while (option != 1) {
      scanf("%d", &option);
...
int createPerson(char type[], int days[]) {
  int regis, option = 0;
  ...
  while (option != 1) {
    ...
    if (type[0] == 'S') {
      scanf("%d", &regis);
      getchar();
      studentsList[qtd_sth].registration = regis;
    } else {
      scanf("%d", &regis);
      getchar();
      teachersList[qtd_tc].registration = regis;
    }

    printf("Nome: \n");
    fgets(type[0] == 'S' ? studentsList[qtd_sth].name : teachersList[qtd_tc].name, 50, stdin);

    printf("Sexo: \n");
    if (type[0] == 'S') {
      scanf("%c", &studentsList[qtd_sth].sex);
      getchar();
    } else {
      scanf("%c", &teachersList[qtd_tc].sex);
      getchar();
    }
    ...
    printf("Dia: ");
    if (type[0] == 'S') {
      scanf("%d", &studentsList[qtd_sth].dateOfBirth[0]);
      getchar();
      days[qtd_sth] = studentsList[qtd_sth].dateOfBirth[0];
    } else {
      scanf("%d", &teachersList[qtd_tc].dateOfBirth[0]);
      getchar();
      days[qtd_tc] = teachersList[qtd_tc].dateOfBirth[0];
    }

    printf("Mês: ");
    if (type[0] == 'S') {
      scanf("%d", &studentsList[qtd_sth].dateOfBirth[1]);
      getchar();
      days[qtd_sth] = days[qtd_sth] + studentsList[qtd_sth].dateOfBirth[1] * 100;
    } else {
      scanf("%d", &teachersList[qtd_tc].dateOfBirth[1]);
      getchar();
      days[qtd_tc] = days[qtd_tc] + teachersList[qtd_tc].dateOfBirth[1] * 100;
    }

    printf("Ano: ");
    if (type[0] == 'S') {
      scanf("%d", &studentsList[qtd_sth].dateOfBirth[2]);
      getchar();
      days[qtd_sth] = days[qtd_sth] + studentsList[qtd_sth].dateOfBirth[2] * 10000;
    } else {
      scanf("%d", &teachersList[qtd_tc].dateOfBirth[2]);
      getchar();
      days[qtd_tc] = days[qtd_tc] + teachersList[qtd_tc].dateOfBirth[2] * 10000;
    }

    if (type[0] == 'S') {
      studentsList[qtd_sth].date = days[qtd_sth];
      studentsList[qtd_sth].status = 1;
    } else {
      teachersList[qtd_tc].date = days[qtd_tc];
      teachersList[qtd_tc].status = 1;
    }

    printf("CPF: \n");
    fgets(type[0] == 'S' ? studentsList[qtd_sth].cpf : teachersList[qtd_tc].cpf, 15, stdin);
...
void getAll(char type[1]) {
  ...
  for (index = 0; index < 50; index++) {
    if (type[0] == 'S' && studentsList[index].status == 1 ||
      ...
      printf("Nome: %s\n",
             type[0] == 'S' ? studentsList[index].name : teachersList[index].name);

      printf("Sexo: %c\n",
             type[0] == 'S' ? studentsList[index].sex : teachersList[index].sex);
      ...
      printf("CPF: %s\n",
             type[0] == 'S' ? studentsList[index].cpf : teachersList[index].cpf);
...
void updatePerson(char type[1], int days[]) {
  ...
  while (stop == 0) {
    ...
    case 2:
      fflush(stdin);
      printf("Digite o novo nome: \n");
      fgets(type[0] == 'S' ? studentsList[index].name : teachersList[index].name, 50,
            stdin);
    ...
    case 3:
      fflush(stdin);
      printf("Digite o novo CPF: \n");
      fgets(type[0] == 'S' ? studentsList[index].cpf : teachersList[index].cpf, 15,
            stdin);
    ...
    if (sex != 'M' && sex != 'F') {
      ...
      for (i = 0; i < 50; i++) {
        ...
        if (type[0] == 'T' && teachersList[i].sex == sex &&
          ...
          printf("Matricula: %d\n", type[0] == 'S' ? studentsList[i].registration
...
void ListTeachersSex(Teacher vetor[]) {
  int option = 2;
  while (option != 1) {
    getchar();
    char sex;
    ...
    if (sex != 'M' && sex != 'F') {
      ...
      int i;
      for (i = 0; i < LEN_TEACHER; i++) {
        if (vetor[i].sex == sex && vetor[i].status == 1) {
          printf("\n>>>>DADOS DO PROFESSOR<<<<\n");
          printf("Nome: %s\n", vetor[i].name);
          printf("Sexo: %c\n", vetor[i].sex);
          printf("Matricula: %d\n", vetor[i].registration);
          printf("Data de Nascimento: %d/%d/%d\n", vetor[i].dateOfBirth[0],
                 vetor[i].dateOfBirth[1], vetor[i].dateOfBirth[2]);
          printf("CPF: %s\n", vetor[i].cpf);
          printf("----------------------------\n");
        }
      }
...
void ListTeachersName(Teacher vetor[]) {

  char *names[50];
  int i;
  for (i = 0; i < LEN_TEACHER; i++) {
    names[i] = vetor[i].name;
  }
...
void ListTeachersDate(Teacher vetor[], int vetor2[]) {
  
  int i, j, valor;
  for (i = 1; i < LEN_TEACHER; i++) {
    valor = vetor2[i];
    j = i - 1;

    while (j >= 0 && vetor2[j] > valor) {
      vetor2[j + 1] = vetor2[j];
      j--;
    }
  vetor2[j + 1] = valor;
  ...
  int x;
  int z;
  printf("\n>>>PROFESSORES LISTADOS POR DATA DE NASCIMENTO<<<\n");
  for(x = 0; x <LEN_TEACHER; x++){
    for(z = 0; z <LEN_TEACHER; z++){
        if (vetor[z].date == vetor2[x] && vetor[z].status == 1)
          printf("%s\n", vetor[z].name);
  }
}
}
...
void teachers() {
  ...
  case 0:
    ...
    break;
  case 1:
    RegisterTeacher(teachersList, days, qtd_tc);
    break;
  case 2:
    changeTeacher(teachersList, days);
    break;
  case 3:
    deletePerson("T");
    break;
  ...
  case 5:
    ...
    break;
  case 6:
    ListTeachersDate(teachersList, days);
    break;
  case 7:
    ListTeachersName(teachersList);
    break;
  default:
    printf("Opção inválida! Digite novamente.\n");
    break;
